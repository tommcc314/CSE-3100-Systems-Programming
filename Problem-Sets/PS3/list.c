...
void sortedInsert(IntList* sortedList, ILNode* nodeToInsert){
    ...
    */
    ...
    else if (sortedList->len == 1) {
        if (nodeToInsert->number >= sortedList->head->number) {
            sortedList->head->next = nodeToInsert;
            nodeToInsert->next = NULL;
            sortedList->tail = nodeToInsert;
        }
        else {
            sortedList->head = nodeToInsert;
            nodeToInsert->next = sortedList->tail;
        }
    }
    else {
        ILNode *prev = sortedList->head;
        ILNode *next = sortedList->head->next;
        if (nodeToInsert->number < prev->number) {
            nodeToInsert->next = prev;
            sortedList->head = nodeToInsert;
        }
            ...
            while (next) {
                if (nodeToInsert->number >= prev->number && nodeToInsert->number < next->number) {
